...
# test
test <- distr[distr$ABO != "   " & distr$ABO != "", ]
test.distr <- aggregate(test$quantity, by = list(test$date), sum); colnames(test.distr) <- c("date", "pcs")
...
The distributions data is not comparable with the sales data. The overall shape of the series is very similar, but the level is wholly different. The similarity might not be enough though, let's do a level correction and see how well our series match then.
...
# Plot with level correction (-4000 to distributions)
ggplot() + 
  geom_line(data = data.new, aes(x = date, y = tot -4000, colour = "distribution"), size = 1) + 
  geom_point(data = data.new, aes(x = date, y = tot -4000, colour = "distribution")) +
  geom_line(data = data.new, aes(x = date, y = sales, colour = "sales"), size = 1) + 
  geom_point(data = data.new, aes(x = date, y = sales, colour = "sales")) +
  scale_color_manual(values = c("#DF013A", "#298A08")) +
  theme(legend.position = "bottom", legend.margin = margin(t = -20, b = 20)) +
...
## Considering missing ABO values
```{r removal_adj}
test.monthly <- aggregate(pcs ~ month(date) + year(date), data = test.distr, FUN = sum)
distr.test <- test.monthly[distr.monthly$`year(date)` >= 2014 & test.monthly$`year(date)` <= 2018, ]$pcs
data.test <- data.frame(date = seq(from = as.Date("2014-01-01"), to = as.Date("2018-12-01"), by = "month"),
                       distr = distr.test,
                       sales = sales[sales$date >= "2014-01-01" & sales$date <= "2018-12-01", ]$Punasoluvalmisteet)
```
...
```{r removal_adj_plots}
ggplot() + 
  geom_line(data = data.test, aes(x = date, y = distr.test, colour = "distribution"), size = 1) + 
  geom_point(data = data.test, aes(x = date, y = distr.test, colour = "distribution")) +
  geom_line(data = data.test, aes(x = date, y = sales, colour = "sales"), size = 1) + 
  geom_point(data = data.test, aes(x = date, y = sales, colour = "sales")) +
  scale_color_manual(values = c("#DF013A", "#298A08")) +
  theme(legend.position = "bottom", legend.margin = margin(t = -20, b = 20)) +
  labs(title = "Distribution minus Returns vs. Sales",
       subtitle = "Distribution does not correspond with sales. Why?",
...
# Plot with level correction (-4000 to distributions)
ggplot() + 
  geom_line(data = data.test, aes(x = date, y = distr.test -4000, colour = "distribution"), size = 1) + 
  geom_point(data = data.test, aes(x = date, y = distr.test -4000, colour = "distribution")) +
  geom_line(data = data.test, aes(x = date, y = sales, colour = "sales"), size = 1) + 
  geom_point(data = data.test, aes(x = date, y = sales, colour = "sales")) +
  scale_color_manual(values = c("#DF013A", "#298A08")) +
  theme(legend.position = "bottom", legend.margin = margin(t = -20, b = 20)) +
...
fwrite(list(es), file = "results/tbats_test_errors.txt")
```
...
fwrite(list(mon.es), file = "results/tbats_test_monthly_errors.txt")
```
