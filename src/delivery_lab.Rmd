...
```{r weekly}
distr.weekly <- aggregate(pcs ~ week(date)+ year(date), data = all.distr, FUN = sum)
distr.weekly <- data.frame(date = seq(from = as.Date("2014-01-01"), 
                                      to = as.Date("2019-06-30"), 
                                      by = "week"), 
                           pcs = distr.weekly[4:343, ]$pcs) # TODO: FIX
```

```{r}
ggplot(data = tail(distr.weekly, 104), aes(x = seq(1, 104), y = pcs)) + geom_line()
```
...
es <- c()
# create progress bar
pb <- txtProgressBar(min = 1095, max = (length(distr.ts) - 1), style= 3)
for(i in seq(from = 1095, to = (length(distr.ts) - 1), by = 30)){
  train <- window(distr.ts,
  ...
  trainlm <- tslm(train ~ trend + season)
  trainlmf <- forecast(trainlm, h = 30)
 
  residauto <- auto.arima(trainlm$residuals)
  residf <- forecast(residauto, h = 30)
...
fwrite(list(es), file = "results/lm_test_errors_30.txt")
```
```{r}
sum(head(tail(es, 91), 30))/head(tail(distr.monthly, 2), 1)$pcs
```

```{r lm_errors, eval = FALSE}
# Sum up LM errors by month
mons <- rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31), 3)
...
```{r}
scaled <- es/distr.ts[1096:2024]

moving_index <- 1
monerrs <- c()
for(i in mons){
  monerr <- sum(scaled[moving_index:(moving_index - 1 + i)])
  moving_index <- moving_index + i
  monerrs <- c(monerrs, monerr)
}
```
...
tbats_es <- c()
# create progress bar
pb <- txtProgressBar(min = 1095, max = (length(distr.mts) - 1), style= 3)
for(i in seq(from = 1095, to = (length(distr.mts) - 1), by = 1)){
  train <- window(distr.mts,
                  start = c(decimal_date(as.Date("2014-01-01"))),
                  end = c(decimal_date(as.Date("2014-01-01")), i))
  test <- distr.mts[i + 1]
...
Rolling windows
All types
All products (platelets and plasma also)
