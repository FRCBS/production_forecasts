...
## Historical demand {.tabset .tabset-fade .tabset-pills}
...
## Weekly forecast {.tabset .tabset-fade .tabset-pills}

### All red
```{r historical_errors, echo = FALSE}
# Define the series to be used
red.mts <- msts(weekly$red, start = decimal_date(as.Date("2014-01-01")), seasonal.periods = c(12, 52))
...
for(i in seq(length(red.mts) - 105)){
  ...
  train <- msts(red.mts[(1 + i):(104 + i)], start = decimal_date(as.Date("2014-01-01") + i), seasonal.periods = c(12, 52))
  test <- msts(red.mts[(105 + i)], start = decimal_date(as.Date("2014-01-01") + 105 + i), seasonal.periods = c(12, 52))
  # Fit
  fit <- tbats(train)
