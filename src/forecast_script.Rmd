...
### All red
```{r historical_errors, message = FALSE, warning = FALSE, echo = FALSE}
...
segment <- tail(monthly$date, 48)
beginning <- head(segment, 1)
...
upper80s <- fcast$upper[[1]]
upper95s <- fcast$upper[[2]]
lower80s <- fcast$lower[[1]]
lower95s <- fcast$lower[[2]]
...
```{r}
class(which.min(colMeans(mdf))[1])
```
```{r}
m
```
...
for(i in seq(to = length(m[,1])-1)){
  name <- names(which.min(m[i, ]))
  colnum <- which.min(m[i, ])[[1]]
  e <- m[i+1, colnum]
  errors <- c(errors, e)
}
