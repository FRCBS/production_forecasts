...
### All red
```{r historical_errors, echo = FALSE, eval = FALSE}
# Define the series to be used
red.ts <- ts(monthly$red, start = decimal_date(as.Date("2014-01-06")), frequency = 12)
...
# Create progress bar
cat("\nRUNNING TBATS\n")
pb <- txtProgressBar(min = 1, max = length(red.ts) - 25, style = 3)
for(i in seq(length(red.ts) - 25)){
  # Define training and testing set as ROLLING WINDOW
  train <- ts(red.ts[(1 + i):(24 + i)], start = decimal_date(as.Date("2014-01-06") + i), frequency = 12)
  test <- ts(red.ts[(25 + i)], start = decimal_date(as.Date("2014-01-06") + 25 + i), frequency = 12)
  # Fit
  fit <- tbats(train)
...
# Create progress bar
cat("\nRUNNING STLF\n")
pb <- txtProgressBar(min = 1, max = length(red.mts) - 105, style = 3)
for(i in seq(length(red.mts) - 105)){
  # Define training and testing set as ROLLING WINDOW
  train <- msts(red.mts[(1 + i):(104 + i)], start = decimal_date(as.Date("2014-01-01")), seasonal.periods = c(12, 52))
  test <- msts(red.mts[(105 + i)], start = decimal_date(as.Date("2014-01-01")), seasonal.periods = c(12, 52))
...
for(i in seq(length(red.mts) - 105)){
  # Define training and testing set as ROLLING WINDOW
  train <- msts(itsmr::smooth.fft(red.mts[(1 + i):(104 + i)], .25), 
...
for(i in seq(length(red.mts) - 105)){
  # Define training and testing set as ROLLING WINDOW
  train <- msts(itsmr::smooth.fft(red.mts[(1 + i):(104 + i)], .10), 
                start = decimal_date(as.Date("2014-01-01")), 
                seasonal.periods = c(12, 52))
  test <- msts(red.mts[(105 + i)], start = decimal_date(as.Date("2014-01-01")), seasonal.periods = c(12, 52))
