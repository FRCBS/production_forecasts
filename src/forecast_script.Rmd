...
## Historical monthly demand {.tabset .tabset-fade .tabset-pills}

### All red
```{r, echo = FALSE}
# We need to extract the past year for determining the trend line
year.segment <- monthly[(length(monthly$red)-12):(length(monthly$red)-1), ]

# Remember to omit the last month, as it is not whole
ggplot(data = head(monthly, -1), aes(x = date, y = red)) + 
  geom_line() +
  geom_smooth(data = year.segment, aes(x = date, y = red), method = "lm", inherit.aes = FALSE) +  # This plots the trend line
  labs(title = "All red cell products",
       subtitle = "Historical demand plus a trend line for the past year",
       caption = paste0("Past year's trend has been ", ifelse(test = lm(year.segment$red ~ seq_along(year.segment$red))$coefficients[2] >= 0,  # Upward or downward? Check the slope!
                                                              yes = "upward.", 
                                                              no = "downward.")),
       x = "Date",
       y = "Blood bags")
...
## Monthly forecast {.tabset .tabset-fade .tabset-pills}

### All red
```{r historical_errors, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
...
red.ts <- ts(segment, start = decimal_date(beginning), frequency = 12)
...
# Create progress bar
cat("\nRUNNING ETS\n")
pb <- txtProgressBar(min = 1, max = length(red.ts) - 37, style = 3)
for(i in seq(length(red.ts) - 37)){
  # Define training and testing set as ROLLING WINDOW
  train <- ts(red.ts[(1 + i):(36 + i)], start = decimal_date(beginning + i), frequency = 12)
  test <- ts(red.ts[(37 + i)], start = decimal_date(beginning + 37 + i), frequency = 12)
  # Fit
  fit <- ets(train)
...
for(i in seq(length(red.ts) - 37)){
  # Define training and testing set as ROLLING WINDOW
  train <- ts(itsmr::smooth.fft(red.ts[(1 + i):(36 + i)], .25), 
...
for(i in seq(length(red.ts) - 37)){
  # Define training and testing set as ROLLING WINDOW
  train <- ts(itsmr::smooth.fft(red.ts[(1 + i):(36 + i)], .10), 
              start = decimal_date(beginning), 
              frequency = 12)
  test <- ts(red.ts[(37 + i)], start = decimal_date(beginning + 37 + i), frequency = 12)
...
train <- ts(tail(red.ts, 36), start = decimal_date(head(tail(monthly$date, 36), 1)), frequency = 12)
train25 <- ts(itsmr::smooth.fft(tail(red.ts, 36), .25), start = decimal_date(head(tail(monthly$date, 36), 1)), frequency = 12)
train10 <- ts(itsmr::smooth.fft(tail(red.ts, 36), .10), start = decimal_date(head(tail(monthly$date, 36), 1)), frequency = 12)
...
ggplot(data = fdf, aes(x = seq.Date(from = tail(monthly$date, 1), to = tail(monthly$date, 1) + months(5), by = "month"), y = fcast)) + 
  geom_line() + 
  geom_line(data = head(monthly, -1), aes(x = date, y = red)) +
  ...
  geom_segment(x = tail(monthly$date, 2)[1], y = tail(monthly$red, 2)[1], xend = tail(monthly$date, 1), yend = head(fdf$fcast, 1), color = "black", linetype = "dashed") +
  labs(title = "6 month forecast",
...
<style>
div.red { background-color:#DF013A; border-radius: 5px; padding: 20px;}
</style>
<div class = "red">

```{r}
paste0("FORECASTED DEMAND FOR NEXT MONTH: ", fdf$fcast[1])
```
