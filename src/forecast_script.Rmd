...
series.ts <- ts(segment, start = c(year(beginning), month(beginning), day(beginning)), frequency = 12) # Again, removing start = decimal_date() due to instability
...
Forecast for `r as.character(lubridate::month(end_date, label = TRUE, abbr = FALSE))` `r lubridate::year(end_date)`: `r format(round(head(fdf_for_plots$fcast, 1), 0))  # This is to round AND display only whole numbers.` platelet units.
...
red_sales_history <- c(monthly_real[monthly_real$date >= as.Date(paste0(year(end_date), "-01-01")), ]$red.sales)
# Remove last month as it will be our first forecast
red_sales_history <- red_sales_history[1:(length(red_sales_history) - 1)]

# Same for returns
red_return_history <- c(monthly_real[monthly_real$date >= as.Date(paste0(year(end_date), "-01-01")), ]$red.ret)
red_return_history <- red_return_history[1:(length(red_return_history) - 1)] * -1

# Get distributions history of the current year
red_history <- c(monthly_real[monthly_real$date >= as.Date(paste0(year(end_date), "-01-01")), ]$red)
# Remove last month as it will be our first forecast
red_history <- red_history[1:(length(red_history) - 1)]

# Now platelets
plat_sales_history <- c(monthly_real[monthly_real$date >= as.Date(paste0(year(end_date), "-01-01")), ]$plat.sales)
plat_sales_history <- plat_sales_history[1:(length(plat_sales_history) - 1)]

plat_return_history <- c(monthly_real[monthly_real$date >= as.Date(paste0(year(end_date), "-01-01")), ]$plat.ret)
plat_return_history <- plat_return_history[1:(length(plat_return_history) - 1)] * -1

plat_history <- c(monthly_real[monthly_real$date >= as.Date(paste0(year(end_date), "-01-01")), ]$plat)
...
if(month(end_date) > 6){
  ...
  red_fcasts <- round(head(fdf_red_for_tables, (13-month(end_date))), 0)
  plat_fcasts <- round(head(fdf_pla_for_tables$fcast, (13-month(end_date))), 0)
}
...
# Sums
red_sales_last <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date) - 1)$red.sales)
red_sales_this <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date))$red.sales) + sum((fdf_red_for_tables[1:(13 - month(end_date)), ]$fcast * red_coeff))
red_sales_next <- sum(fdf_red_for_tables[(14 - month(end_date)):(26 - month(end_date)), ]$fcast * red_coeff)

red_last <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date) - 1)$red)
red_this <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date))$red) + sum(fdf_red_for_tables[1:(13 - month(end_date)), ]$fcast)
red_next <- sum(fdf_red_for_tables[(14 - month(end_date)):(26 - month(end_date)), ]$fcast)

red_ret_last <- red_last - red_sales_last
red_ret_this <- red_this - red_sales_this
red_ret_next <- red_next - red_sales_next

plat_sales_last <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date) - 1)$plat.sales)
plat_sales_this <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date))$plat.sales) + sum((fdf_pla_for_tables[1:(13 - month(end_date)), ]$fcast * plat_coeff))
plat_sales_next <- sum(fdf_pla_for_tables[(14 - month(end_date)):(26 - month(end_date)), ]$fcast * plat_coeff)

plat_last <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date) - 1)$plat)
plat_this <- sum(subset(monthly_real, format(as.Date(date), "%Y") == year(end_date))$plat) + sum(fdf_pla_for_tables[1:(13 - month(end_date)), ]$fcast)
plat_next <- sum(fdf_pla_for_tables[(14 - month(end_date)):(26 - month(end_date)), ]$fcast)
...
Forecast for week `r week(end_date)` of `r lubridate::year(end_date)`: `r format(round(head(fdf_for_plots$fcast, 1), 0))  # This is to round AND display only whole numbers.` blood bags.
