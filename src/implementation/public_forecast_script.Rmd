...
if (RES == "monthly") {
    ...
    fcast_workdays <- as.numeric(bizdays(ts(1:N, start = c(year(fcast_start_date), month(fcast_start_date), day(fcast_start_date)), 
                                            frequency = 12),
                                         FinCenter = "Zurich"))
    # Adjust series
...
if (RES == "monthly") {
    ...
    fcast_start_date <- Ominus_monthly[nrow(Ominus_monthly), 1] + months(1) # For adjusting the forecast
