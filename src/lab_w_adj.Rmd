...
There is significant autocorrelation at multiple lags, which tells us that there is a considerable amount of information thrown out during the decomposition. This is not a particularly good decomposition. Let's check decompositions for platelets and plasma also.
...
Jarno built his forecasts using **STL+ETS** and **ETS** models, including also a naïve 6 month repetition forecast. I will not be considering the naïve "forecast" here, as I'm fairly certain we want to do proper modelling. 
```{r}
...
for(i in seq(from=2008, to=2018, by=1)){
  fit <- stl(window(ts.red, start=2004, end=c(i-1, 12)), s.window="periodic", t.window=7)  # Fit based on history so far
...
# Loop 
for(i in seq(from=2008, to=2018, by=1)){
  fit <- stl(window(ts.pla, start=2004, end=c(i-1, 12)), s.window="periodic", t.window=7)  # Fit based on history so far
  fcast <- forecast(fit, h=12)  # Forecast the next year
...
# Loop 
for(i in seq(from=2008, to=2018, by=1)){
  fit <- stl(window(ts.ffp, start=2004, end=c(i-1, 12)), s.window="periodic", t.window=7)  # Fit based on history so far
  fcast <- forecast(fit, h=12)  # Forecast the next year
