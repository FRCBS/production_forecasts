...
```{r}
start_init <- as.Date("2004-01-01")
end_init <- as.Date("2012-01-01")
end_of_time <- monthly$date[length(monthly$date)]
...
for(i in 0:nmonths){
  ...
  metricvec <- append(metricvec, features(segment, value, features = feat_stl)[[2]])
}
plot(1:length(metricvec), metricvec)
```
```{r}
plot(monthly$date, monthly$red, "l")
```
